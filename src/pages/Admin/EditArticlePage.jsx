...
const EditArticlePage = () => {
  ...
  useEffect(() => {
    const fetchArticleDetails = async () => {
      try {
        ...
        setFormData({
          ...response[0], 
          authorId: response[0].authorId._id, 
          subCategoryId: response[0].subCategoryId._id, 
        });
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await articleApi.updateArticle(articleId, formData); // Use update API
  ...
  return (<>
    <Container>
      <Header />
      <h1 style={{ textAlign: 'center' }}>Update Article</h1>
